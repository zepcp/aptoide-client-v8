...
public class RollbackInstallManager implements Installer {
	...
	public Observable<Void> install(Context context, PermissionRequest permissionRequest, long installationId) {
		return installationProvider.getInstallation(installationId)
				.flatMap(installation -> rollbackProvider.createRollback(installation, Rollback.Action.INSTALL))
				.map(rollback -> {
	...
	@Override
	public Observable<Void> update(Context context, PermissionRequest permissionRequest, long installationId) {

		return installationProvider.getInstallation(installationId)
				.concatMap(installation -> getRollbackObservable(installation.getPackageName(), Rollback.Action.UPDATE, installation.getIcon()))
				.observeOn(AndroidSchedulers.mainThread())
				.concatWith(installManager.update(context, permissionRequest, installationId));
	}

	@Override
	public Observable<Void> downgrade(Context context, PermissionRequest permissionRequest, long installationId) {
		return installationProvider.getInstallation(installationId)
				.concatMap(installation -> getRollbackObservable(installation.getPackageName(), Rollback.Action.DOWNGRADE, installation.getIcon()))
				.observeOn(AndroidSchedulers.mainThread())
	...
	public Observable<Void> uninstall(Context context, String packageName) {
		return rollbackProvider.createRollback(context, packageName, Rollback.Action.UNINSTALL, null).map(rollback -> {
			repository.save(rollback);
			return rollback;
		}).concatMap(rollback -> installManager.uninstall(context, packageName));
	}

	private Observable<Void> getRollbackObservable(String packageName, Rollback.Action action, String icon) {
		return rollbackProvider.createRollback(Application.getContext(), packageName, action, icon).map(rollback -> {
			repository.save(rollback);
			return null;
		});
	}
