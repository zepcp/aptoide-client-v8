...
public class UpdateDisplayable extends Displayable {
	...
	public static UpdateDisplayable create(Update update, Installer installManager, DownloadFactory downloadFactory, DownloadServiceHelper downloadManager) {
		...
		return new UpdateDisplayable(update.getPackageName(), update.getAppId(), update.getLabel(), update.getIcon(), update.getVersionCode(), update.getMd5()
				...
				.getPatchObbName(), update.getPatchObbPath(), update.getPatchObbMd5(), installManager, downloadFactory.create(update), downloadManager, null,
				null);
	}
	...
	public Observable<Void> downloadAndInstall(Context context) {
		...
		return downloadManager.startDownload((PermissionRequest) context, download)
				...
				.concatMap(downloadCompleted -> installManager.update(context, (PermissionRequest) context, download.getAppId()));
	}
